...
void updateNode(int x, int y, square current, std::pair<int, int> end_coord) {
    ...
    if(y >= 0 && y < grid.size() && x >= 0 && x < grid[0].size() 
        ...
        neighbour.h_cost = manhattan_dist(neighbour.coord, end_coord);
        neighbour.f_cost = neighbour.g_cost + neighbour.h_cost;
        openSet.push(neighbour);
    }   
}

void updateNeighbours(square current, std::pair<int, int> end_coord) {
    if(closedSet.count(current.coord)) //catches duplicates in openSet
        return;
    updateNode(current.coord.first, current.coord.second - 1, current, end_coord);      //North
    updateNode(current.coord.first + 1 , current.coord.second, current, end_coord);     //East
    updateNode(current.coord.first, current.coord.second + 1, current, end_coord);      //South
    updateNode(current.coord.first - 1, current.coord.second, current, end_coord);      //West
}
...
int main(){
    ...
    square start_square(start_coord);
    start_square.g_cost = 0;
    start_square.h_cost = manhattan_dist(start_coord, end_coord);
    ...
    while(true) {
        ...
        openSet.pop();
        updateNeighbours(smallest, end_coord);
